...
def get_weekly_title():
    """오늘 날짜를 기준으로 이번 주 월요일~일요일 문자열을 생성합니다."""
    today = datetime.date.today()
    # today.weekday()는 월요일이 0, 일요일이 6
...
st.markdown("""
<style>
    :root { --primary-purple: #6c5ce7; --light-purple: #f0eeff; }
    
    /* 사이드바 너비 고정 */
    section[data-testid="stSidebar"] { min-width: 350px !important; max-width: 350px !important; }

    /* [수정] 사이드바 안의 버튼들 기본 스타일 (아이콘 버튼용) */
    /* 텍스트 버튼(타이틀)과는 구별하기 위해 특정 클래스 없는 버튼만 타겟팅 */
    [data-testid="stSidebar"] .stButton:not(.title-btn) button {
    ...
    .save-btn-container .stButton button {
        background-color: var(--primary-purple) !important; color: white !important; font-size: 18px !important; font-weight: bold !important; padding: 12px 40px !important; border-radius: 50px !important; border: none !important; box-shadow: 0 4px 15px rgba(108, 92, 231, 0.3) !important; transition: transform 0.1s;
    }
    .save-btn-container .stButton button:hover { transform: scale(1.05); background-color: #5b4cc4 !important; }
...
with st.sidebar:
    ...
    # [수정] 새 주간 시작하기 버튼 (꽉 찬 버튼 스타일)
    # type="primary"와 use_container_width=True를 사용하여 꽉 찬 유색 버튼으로 만듦
    if st.button("➕ 새 주간 시작하기", key="new_week", type="primary", use_container_width=True):
    ...
    for i, item in enumerate(st.session_state.history):
        ...
        # [핵심 수정] st.container(border=True)를 사용하여 확실한 테두리 박스 생성
        with st.container(border=True):
            # 레이아웃: [ 텍스트 영역 (70%) ] [ 복사 (15%) ] [ 삭제 (15%) ]
            c_text, c_copy, c_del = st.columns([0.7, 0.15, 0.15])
            
            # 1. 텍스트 영역 (타이틀 버튼 + 목표 캡션)
            with c_text:
                # 타이틀을 버튼으로 만들어서 클릭 기능 제공 (CSS로 텍스트처럼 보이게 함)
            ...
            with c_copy:
                ...
                # [복사 로직 수정] 복사 시에도 날짜는 자동 생성된 새 날짜로 부여
                if st.button("📋", key=f"copy_{i}", help="이 기록 복사해서 새 주간 만들기"):
                    new_item = item.copy()
                    new_item['id'] = str(datetime.datetime.now().timestamp())
                    # 복사할 때도 새로운 주간 타이틀 자동 생성
...
if "current_data" not in st.session_state or st.session_state.current_data is None:
    # [핵심 수정] 초기 데이터 생성 시 자동 날짜 함수 사용
    today_str = get_weekly_title()
...
# [수정] 날짜/제목 영역을 수정 불가능한 헤더로 변경하고 목표만 입력받음
st.subheader(f"📅 {data['title']}") # 날짜는 고정된 큰 제목으로 보여줌
